   verbose <- FALSE
    ParNew <- update_astrometry(Par,ParFit)
...
###RV model
        ...
        if(length(index)>0){
            RvKep <- fit_RvKep(OutTimeNew,Data,OutBaryNew,ParFit,Par,star)
            ModelKep[index,2] <- RvKep[index]
            RvTrend <- fit_RvTrend(OutTimeNew,Data,ParFit,Par,star)
            ModelTrend[index,2] <- RvTrend[index]
            RvHat <- RvKep+RvTrend
            RvRed <- fit_RvRed(OutTimeNew,RvHat,Data,ParFit,Par,star)
            ModelRed[index,2] <- RvRed[index]
            RvHat <- RvHat+RvRed
            model[index,2] <- RvHat[index]
            llike <- fit_RvLike(RvHat,Data,ParFit,Par,star)
...
###absolute astrometry model
        ...
        if(length(ind)>0){
            AstroList[[star]] <- AstroKep <- fit_AstroKep(OutTimeNew,Data,OutBaryNew,ParFit,Par,star)#rad
            if(length(index)>0){
                ModelKep[index,c(2,4)] <- AstroKep[index,]
                AstroTrend <- fit_AstroTrend(OutTimeNew,Data,ParFit,Par,star=star,type='abs')#rad
                ModelTrend[index,c(2,4)] <- AstroTrend[index,]
                AstroHat <- AstroKep+AstroTrend
                AstroRed <- fit_AstroRed(OutTimeNew,AstroHat,Data,ParFit,Par,star=star,type='abs')#rad
                ModelRed[index,c(2,4)] <- AstroRed[index,]
                AstroHat <- AstroHat+AstroRed
                model[index,c(2,4)] <- AstroHat[index,]
                llike <- fit_AstroLike(AstroHat,Data,ParFit,Par,star=star,type='abs')
...
###relative astrometry model
    ...
    if(length(index)>0 & ParNew$Np>0){
        ...
        ModelKep[index,c(2,4)] <- AstroKep[index,]
        AstroTrend <- fit_AstroTrend(OutTimeNew,Data,ParFit,Par,star,type='rel')
        ModelTrend[index,c(2,4)] <- AstroTrend[index,]
        AstroHat <- AstroKep+AstroTrend
        AstroRed <- fit_AstroRed(OutTimeNew,AstroHat,Data,ParFit,Par,star,type='rel')
        ModelRed[index,c(2,4)] <- AstroRed[index,]
        AstroHat <- AstroHat+AstroRed
        model[index,c(2,4)] <- AstroHat[index,]
        llike <- fit_AstroLike(AstroHat,Data,ParFit,Par,star,type='rel')
