...
RateObs <- update_NumDerivObs(utc,Par,ParOpt)
#fit <- fit_LogLike(Data,OutObs,RateObs=RateObs,ParFit=ParOpt,Par=Par,OutTime0=OutTime,TimeUpdate=FALSE)
fit <- fit_LogLike(Data,OutObs,RateObs=RateObs,ParFit=ParOpt,Par=Par,OutTime0=OutTime,TimeUpdate=TRUE)
model <- fit$model
...
####plot
if(!exists('bary.type')) bary.type <- 'sum'
fname <- paste0(dir.out,opt$primary,'_',opt$Companion,'companion_',opt$component,'_llmax',llmax,'_N',Par$Niter,'_einstein',Par$Einstein,'_Nrv',Nrv,'_',bary.type,'bary')
