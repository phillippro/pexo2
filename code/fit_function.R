...
###June 25, 2021, add logmT as a free parameter
...
####################################
    ...
    if(!is.null(Par$logmT)){
       ParNew$mC1 <- exp(Par$logmT)
       ParNew$logmC1 <- Par$logmT
    }else{
       ParNew$mC1 <- Par$mT
...
##assume uniform distribution
    for(n in names(ParFit)){
        ...
        if(length(ind)>0){
            ...
            }else if(Par$prior[ind]=='sinU'){
###usually is inclination
	    	lp <- abs(sin(ParFit[n]))
            }else{
...
###Parallel signal identification and constraints
	...
	f3 <- list.files(path='../pars',pattern=paste0('^',Par$target,'\\.par$'),full.name=TRUE)
        ...
        if(length(f3)>0){
###Use input parameter values as initial values
            dd <- read.table(f3)
            parname <- dd[,1]
            par <- dd[,2]
            names(par) <- parname
...
###update the offsets to optimize initial values
...
###use hot chains to find optimize initial values
#        if(length(f3)==0){
...
####################################
    ...
    kep.name <- c('logmT','mT','logmC','mC','logP','P','e','I','Omega','omegaT','omegaC','Mo','Tp','Tc','Tasc')
