...
if(any(cn=='xtel') & !any(cn==xn)) Par[[xn]] <- Par$xtel
if(any(cn=='ytel') & !any(cn==yn)) Par[[yn]] <- Par$ytel
if(any(cn=='ztel') & !any(cn==zn)) Par[[zn]] <- Par$ztel
#if(any(cn=='bRv')) Par[[zn]] <- Par$ztel
if(any(cn=='elong') &  !any(cn==en))	Par[[en]] <- Par$elong*pi/180#input is in degree and thus convert to radians
if(any(cn=='phi') &  !any(cn==pn)) Par[[pn]] <- Par$phi*pi/180
if(any(cn=='height') &  !any(cn==hn)) Par[[hn]] <- Par$height#km
if(any(cn=='ObsType') & !any(cn==tn)) Par[[tn]] <- Par$ObsType
if(any(cn=='observatory') & !any(cn==obsn)){
    Par[[obsn]] <- Par$observatory
}else{
    Par[[obsn]] <- ins
}
cn <- names(Par)
if(any(cn=='ObsCode') &  !any(cn==coden)) Par[[coden]] <- Par$ObsCode
if(any(cn=='SpaceObs') &  !any(cn==sn)) Par[[sn]] <- Par$SpaceObs
Par[[tn]] <- 'ground'#Observatory type: ground (default) or space
if(any(cn==xn) & any(cn==yn) & any(cn==zn) & (!any(cn==en) | !any(cn==pn) |  !any(cn==hn))){
####customerized gound based observatory data
    eph <- sofa_Gc2gd(2,xyz=c(Par[[xn]]/1e3,Par[[yn]]/1e3,Par[[zn]]/1e3))
    Par[[en]] <- eph['elong']#rad
    Par[[pn]] <- eph['phi']#rad
    Par[[hn]] <- eph['height']/1e3#km
}else if(any(cn==en) & any(cn==pn) & any(cn==hn) & (!any(cn==xn) | !any(cn==yn) | !any(cn==zn))){
####customerized gound based observatory data
    xyz <- as.numeric(sofa_Gd2gc(Par$n,elong=Par[[en]],phi=Par[[pn]],height=Par[[hn]]*1e3))
    Par[[xn]] <- xyz[1]/1e3#km
    Par[[yn]] <- xyz[2]/1e3#
    Par[[zn]] <- xyz[3]/1e3
}else if(!(any(cn==en) & any(cn==pn) & any(cn==hn)) & !(any(cn==en) & any(cn==pn) & any(cn==hn))){
