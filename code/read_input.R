...
#opt$time <- '2450000 2460000 10'
#opt$time <- '2445000 2465000 10'
#opt$time <- '../input/HD128620_HARPSpre.tim'
#opt$time <- '../input/HD128620_PFS.tim'
opt$time <- '../input/HD128620_AAT.tim'
#opt$time <- '2440000 2460000 10'
#opt$time <- '2452850 2457498 1'
opt$mode  <- 'emulate'
#opt$mode  <- 'fit'
...
opt$orbit <- NULL
if(opt$Companion>0){
   if(is.null(opt$orbfile)) opt$orbfile <- paste0('../input/',opt$primary,'.orb')
   if(file.exists(opt$orbfile)){
       orbit <- rep(NA,8)
       names(orbit) <- nn <- c('logmT','logmC','logP','e','I','omegaT','Omega','Tp')
       if(opt$Companion>0){
           ...
           for(i in 1:nrow(ff)){
               ...
               }else if(n=='Einstein' | n=='Lensing' | ff[i,2]=='TRUE' | ff[i,2]=='FALSE'){
...
tmp <- gsub('#.+','',tmp)
tmp <- tmp[tmp!='']
if(!any(grepl('elong|xtel',tmp))) autofind <- TRUE
if(autofind) source('add_par.R')
##input parameters
for(p in tmp){
    s <- unlist(strsplit(p,' '))
    s <- s[s!='']
#    if(!any(names(Par)==s[1])){
        if(!is.na(suppressWarnings(as.numeric(s[2])))){
            Par[[s[1]]] <- as.numeric(s[2])
            if(grepl('phi|elong',s[1])) Par[[s[1]]] <- Par[[s[1]]]*pi/180
        }else{
            Par[[s[1]]] <- s[2]
        }
        if(length(s)>2){
        ...
        }else if(any(s[1]==c('mC','mT','logmT','logmC','mTC','mCT','mT','aC','aT','aTC','aCT','omegaT','omegaC','omegaT','omegaC','Omega','I','Mo','Tc','Tasc','Tp','T0','e','P','logP')) & Par$Companion>0){
            Par$Ini[[s[1]]] <- as.numeric(s[2])
            Par$PriorPar1[[s[1]]] <- as.numeric(s[2])-100
            Par$PriorPar2[[s[1]]] <- as.numeric(s[2])+100
            if(s[1]!='I'){
                Par$prior[[s[1]]] <- 'U'
            }else{
                Par$prior[[s[1]]] <- 'sinU'
...
ra0 <- Par$ra <- Par$ra*pi/180
dec0 <- Par$dec <- Par$dec*pi/180
...
if(Par$binary){
    ...
    ind <- which('logmT'==names(Par$Max))
    if(length(ind)>0){
        Par$KepIni['logmT'] <- Par$Ini['logmT']
        Par$KepMax['logmT'] <- Par$Max['logmT']
        Par$KepMin['logmT'] <- Par$Min['logmT']
        Par$KepPrior['logmT'] <- Par$prior['logmT']
        Par$KepPar1['logmT'] <- Par$PriorPar1['logmT']
        Par$KepPar2['logmT'] <- Par$PriorPar2['logmT']
    }else{
        Par$KepIni['logmT'] <- Par$logmT
        Par$KepMin['logmT'] <- -10
        Par$KepMax['logmT'] <- 20
        Par$KepPrior['logmT'] <- 'U'
        Par$KepPar1['logmT'] <- Par$KepMin['logmT']
        Par$KepPar2['logmT'] <- Par$KepMax['logmT']
    }
    ...
    Par$I <- Par$I#rad
    Par$Ini['I'] <- Par$Ini['I']#rad
    Par$Min['I'] <- Par$Min['I']#rad
    Par$Max['I'] <- Par$Max['I']#rad
    ...
    if(length(indT)>0){
        Par$omegaT <- Par$omegaT
        Par$omegaC <- (Par$omegaT+pi)%%(2*pi)
    }
    if(length(indC)>0){
        Par$omegaC <- Par$omegaC
        Par$omegaT <- (Par$omegaC+pi)%%(2*pi)
    }
    Par$Ini['omegaT'] <- Par$Ini['omegaT']
    Par$Min['omegaT'] <- Par$Min['omegaT']
    Par$Max['omegaT'] <- Par$Max['omegaT']
    ...
    Par$Omega <- Par$Omega
    Par$Ini['Omega'] <- Par$Ini['Omega']
    Par$Min['Omega'] <- Par$Min['Omega']
    Par$Max['Omega'] <- Par$Max['Omega']
    ...
    if(length(ind)>0){
        Par$KepIni['Mo'] <- Par$Ini['Mo']
        Par$KepMax['Mo'] <- Par$Max['Mo']
        Par$KepMin['Mo'] <- Par$Min['Mo']
